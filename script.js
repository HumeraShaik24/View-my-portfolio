...
const highlightSections = ['about', 'experience', 'mywork'];
const navLinksAll = document.querySelectorAll('nav a');

window.addEventListener('scroll', () => {
    let highlight = false;
    highlightSections.forEach(id => {
    ...
    navLinksAll.forEach(link => {
        if (highlight) {
            link.classList.add('bookmark-highlight');
        } else {
            link.classList.remove('bookmark-highlight');
        }
    });
});